...
def validate_position(position: Dict[str, Any], position_index: int) -> Tuple[bool, Optional[str]]:
    """
    ...
    if missing_fields:
        return False, f'Position {position_index}: Missing mandatory field(s): {", ".join(missing_fields)}'

    # Validate exchange
    if position.get('exchange') not in VALID_EXCHANGES:
        return False, f'Position {position_index}: Invalid exchange. Must be one of: {", ".join(VALID_EXCHANGES)}'
    ...
    if 'action' in position:
        ...
        if position['action'] not in VALID_ACTIONS:
            return False, f'Position {position_index}: Invalid action. Must be one of: {", ".join(VALID_ACTIONS)}'

    # Validate price type
    if position.get('pricetype') not in VALID_PRICE_TYPES:
        return False, f'Position {position_index}: Invalid price type. Must be one of: {", ".join(VALID_PRICE_TYPES)}'

    # Validate product type
    if position.get('product') not in VALID_PRODUCT_TYPES:
        return False, f'Position {position_index}: Invalid product type. Must be one of: {", ".join(VALID_PRODUCT_TYPES)}'
...
def validate_margin_data(data: Dict[str, Any]) -> Tuple[bool, Optional[List[Dict[str, Any]]], Optional[str]]:
    """
...
def calculate_margin_with_auth(
    ...
    if status_code == 200:
        ...
        message = response_data.get('message', 'Failed to calculate margin') if isinstance(response_data, dict) else 'Failed to calculate margin'
        error_response = {
            'status': 'error',
            'message': message
        }
...
def calculate_margin(
    ...
    if api_key and not (auth_token and broker):
        ...
        return calculate_margin_with_auth(validated_positions, AUTH_TOKEN, broker_name, original_data)
